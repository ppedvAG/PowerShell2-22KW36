...
try
{
Get-EventLog -LogName Security -ComputerName $ComputerName -ErrorAction Stop | Where-Object EventId -eq $EventId | Select-Object -First $Newest
}
catch
{
    $LogPath = (Get-Location).Path + "\ErrorLog.txt"
    Out-File -InputObject $Error[0] -FilePath $LogPath
}
